...
def train(args, loaders, run_dirs, reporter=None, verbose=True):
    ...
    model = VAEFreeGAN(con_dim, bin_dim, config, loaders.num_classes).to(device)
    model.train()
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        for xc, xb, xa, yb in iterate_training_batches(loaders, train_loader=train_loader, sampler=sampler, shuffle=sampler is None):
            ...
            with _AutocastAdaptor(amp_device_type, use_amp):
                xt_real, t = model.diffusion(xa)
                x_real = config.alpha * xa + (1.0 - config.alpha) * xt_real
                noise = model.sample_noise(xa.size(0), device)
                fake_out = model.generator(noise, t, yb)
                x_fake = compose_mixed_feature_tensor(fake_out, discrete_meta)

                x_real_detached = x_real.detach()
                if config.use_r1_penalty:
                    x_real_detached.requires_grad_(True)
                adv_real, cls_real = model.discriminator(x_real_detached, t, yb)
                adv_fake, _ = model.discriminator(x_fake.detach(), t, yb)
            ...
            with _AutocastAdaptor(amp_device_type, use_amp):
                adv_fake_g, cls_fake_g = model.discriminator(x_fake, t, yb)
