...
class VanillaGANConfig:
    ...
    def load_config(self, config_path, verbose=True):
        ...
        if verbose:
            tqdm.write(summarize_model_config("VanillaGAN", self))
            if ignored_keys:
                tqdm.write(f"[WARN] ignored config keys: {', '.join(sorted(ignored_keys))}")


class VanillaFeatureGenerator(nn.Module):
    def __init__(self, noise_dim, con_dim, bin_dim, num_classes,
                 hidden_dim=256, num_blocks=2, y_emb_dim=16,
                 shared_dim=64, head_hidden_dim=64):
        super().__init__()
        self.y_emb = nn.Embedding(num_classes, y_emb_dim)
        in_dim = noise_dim + y_emb_dim
...
class VanillaFeatureDiscriminator(nn.Module):
    def __init__(self, input_dim, num_classes,
                 hidden_dim=256, num_blocks=2, y_emb_dim=16, use_spectral_norm=True):
        super().__init__()
        self.y_emb = nn.Embedding(num_classes, y_emb_dim)
        in_dim = input_dim + y_emb_dim
...
class VanillaGAN(nn.Module):
    def __init__(self, con_dim, bin_dim, config, num_classes):
        ...
        self.generator = VanillaFeatureGenerator(
            config.noise_dim, con_dim, bin_dim, num_classes,
            hidden_dim=config.generator_hidden_dim,
            num_blocks=config.generator_num_blocks,
            y_emb_dim=config.class_embedding_dim,
            shared_dim=config.generator_shared_dim,
            head_hidden_dim=config.generator_head_hidden_dim )
        
        self.discriminator = VanillaFeatureDiscriminator(
