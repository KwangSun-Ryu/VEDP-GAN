...
def _build_model_from_checkpoint(ckpt, device):
    ...
    model = DiffusionOnlyModel(con_dim, bin_dim, config, ckpt["num_classes"]).to(device)
...
def sample(args, loaders, run_dirs, ckpt_path=None, model=None, session=None, return_frame=False, reporter=None):
    ...
    def _generate_for(label_value, num_rows):
        ...
        z = torch.randn(num_rows, model.encoder.mu.out_features, device=device)
        with torch.no_grad():
            for step in reversed(range(model.timesteps)):
                t = torch.full((num_rows,), step, dtype=torch.long, device=device)
                noise_pred = model.denoiser(z, t, labels)
                z = model.schedule.p_sample(z, t, noise_pred)
                if detail_bar is not None:
                    detail_bar.update(1)
                    detail_bar.set_postfix({"label": label_value, "t": step}, refresh=True)
            out = model.decoder(z)
    ...
    detail_total = len(sampling_plan) * max(1, getattr(model, "timesteps", 1))
