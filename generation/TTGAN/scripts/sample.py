...
def create_args():
    ...
    parser.add_argument('--pred-model-name', type=str,
                    ...
                    help='Select which model(s) to use, separated by spaces')
    ...
    parser.add_argument('--num_data', type=int, help='Number of rows to generate')
...
def load_data_info(args):
    """Load column information and column mapping information."""
...
def load_generator(args):
    ...
    if hasattr(model, 'models'):  # legacy class-wise wrapper
...
def load_decoding_resources(args):
    """Load the encoder/discretizer required for data restoration."""
...
def sample(args, save=True, output_path=None, verbose=True):
    ...
    # 3) Load encoder and discretizer
    ...
    # Discretized columns are sampled with gumbel_softmax(hard=False),
    # so they are output as real numbers close to integers instead of exact integers.
    # Therefore, they must be converted back to discretized integer values.
    ...
    # 5) Convert to continuous values using predictors
    predicted = {}
    predictor_dir = os.path.join(args.exp_dir, 'TTGAN', args.data_name, 'predictors', args.pred_model_name)
    ...
    # Convert continuous variables that were int64 back to int64
    columns = json.load(open(os.path.join(args.data_dir, 'TTGAN_data', args.data_name, 'columns.json'), 'r', encoding="utf-8"))
    
    dtype_map = {info['name']: info['dtype'] for info in columns}
    
    # Add noise to continuous variables when needed
    ...
    # Convert continuous variables to integers when their dtype is int64
    ...
    # 6) Inverse-transform categorical variables to their original values
