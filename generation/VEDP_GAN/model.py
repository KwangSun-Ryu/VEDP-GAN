"""Canonical VEDP_GAN architecture.
...
and the final VEDP_GAN module. Experiment orchestration, checkpoint selection, and
...
class VEDP_GANConfig:
    """Configuration object for the canonical VEDP_GAN architecture."""
    ...
    def load_config(self, config_path, verbose=True):
        ...
        if verbose:
            tqdm.write(summarize_model_config("VEDP_GAN", self))
            if ignored_keys:
...
class VEDP_GAN(nn.Module):
    """VEDP_GAN model: VAE encoder/decoder, latent diffusion, and GAN heads."""
