...
def has_split(path: Union[str, Path], split: str) -> bool:
    path = Path(path)
    return (path / f'y_{split}.npy').exists()


def available_splits(path: Union[str, Path]) -> List[str]:
    path = Path(path)
    return [split for split in ['train', 'val', 'test'] if has_split(path, split)]
...
def change_val(dataset: Dataset, val_size: float = 0.2):
    """Ensure the dataset has a validation split by re-partitioning train/val."""
    if val_size <= 0:
        return dataset

    y_train = dataset.y['train']
    existing_val = dataset.y.get('val')

    if existing_val is not None and len(existing_val):
        y_full = np.concatenate([y_train, existing_val], axis=0)
    else:
        y_full = y_train
    ...
    if not dataset.is_regression:
        unique = np.unique(y_full)
        if unique.shape[0] > 1:
            stratify = y_full

    train_ixs, val_ixs = train_test_split(
        np.arange(y_full.shape[0]),
        test_size=val_size,
        random_state=777,
        stratify=stratify
    )

    dataset.y['train'] = y_full[train_ixs]
    dataset.y['val'] = y_full[val_ixs]

    def _reassign(storage: Optional[ArrayDict]):
        if storage is None:
            return
        train_arr = storage['train']
        val_arr = storage.get('val')
        if val_arr is not None and len(val_arr):
            combined = np.concatenate([train_arr, val_arr], axis=0)
        else:
            combined = train_arr
        storage['train'] = combined[train_ixs]
        storage['val'] = combined[val_ixs]

    _reassign(dataset.X_num)
    _reassign(dataset.X_cat)
...
def normalize(
    ...
    def _transform(arr: np.ndarray) -> np.ndarray:
        if arr.shape[0] == 0:
            return arr.copy()
        return normalizer.transform(arr)

    transformed = {k: _transform(v) for k, v in X.items()}
...
def read_pure_data(path, split='train'):
    ...
    if not y_path.exists():
        raise FileNotFoundError(f"Split '{split}' not found in {path}")

    y = np.load(y_path, allow_pickle=True)
    X_num = None
    X_cat = None

    num_path = path / f'X_num_{split}.npy'
    if num_path.exists():
        X_num = np.load(num_path, allow_pickle=True)
    cat_path = path / f'X_cat_{split}.npy'
    if cat_path.exists():
...
def read_changed_val(path, val_size=0.2):
    ...
    has_val = has_split(path, 'val')
    if has_val:
        X_num_val, X_cat_val, y_val = read_pure_data(path, 'val')
        y_full = np.concatenate([y_train, y_val], axis=0)
    else:
        X_num_val = X_cat_val = None
        y_val = None
        y_full = y_train

    if y_full.shape[0] < 2 or val_size <= 0:
    ...
    y_train_new = y_full[train_ixs]
    y_val_new = y_full[val_ixs]

    def _split_arrays(train_arr, val_arr):
        ...
        if val_arr is not None and len(val_arr):
            ...
            combined = train_arr
        return combined[train_ixs], combined[val_ixs]

    X_num_train_new, X_num_val_new = _split_arrays(X_num_train, X_num_val)
    X_cat_train_new, X_cat_val_new = _split_arrays(X_cat_train, X_cat_val)

    return X_num_train_new, X_cat_train_new, y_train_new, X_num_val_new, X_cat_val_new, y_val_new
#############
