...
def _to_tensor_like(val, ref=None):
    ...
    if torch.is_tensor(val):
        ...
        out = torch.as_tensor(val)
    if ref is not None and torch.is_tensor(ref):
        target_dtype = ref.dtype if ref.dtype.is_floating_point else torch.float32
        out = out.to(device=ref.device, dtype=target_dtype)
    elif not out.dtype.is_floating_point:
...
def marginal_prob_mean(x, t, beta_1, beta_0):
    ...
    b0 = _to_tensor_like(beta_0, ref=x)
    b1 = _to_tensor_like(beta_1, ref=x)
    log_mean_coeff = -0.25 * t ** 2 * (b1 - b0) - 0.5 * t * b0
...
def min_max_scaling(factor, scale=(0, 1)):
    ...
    new_min = torch.as_tensor(scale[0], dtype=factor.dtype, device=factor.device)
    new_max = torch.as_tensor(scale[1], dtype=factor.dtype, device=factor.device)
