...
class StandardScaler(object):
    ...
    def fit(self, x):
        ...
        self.normalizer.fit(x.to_numpy().reshape(-1, 1))
        return self
    
    def transform(self, x):
        Quantized = self.normalizer.transform(x.to_numpy().reshape(-1, 1))
...
class DataFrameParser(object):
    ...
    def __init__(self, max_cardinality=128):
        ...
        self.generated_binary_info = dict()
        self.generated_categorical_info = dict()
        self._original_columns = list()
        
    def fit(self, dataframe, threshold):
        
        self.new_dataframe = dataframe.copy()
        self._original_columns = list(self.new_dataframe.columns)
        ...
        for column, datatype in column_to_dtype.items():
            if datatype in ['O', '<U32']:
                ...
                if len(repeated_entries) == 1:
                    ...
                    self.generated_binary_info[new_column_name] = {
                        'source': column,
                        'values': repeated_entries
                    }
                ...
                if len(repeated_entries) >= 2 and len(repeated_entries) <= 25:
                    ...
                    self.generated_categorical_info[new_column_name] = {
                        'source': column,
                        'values': repeated_entries
                    }
                    self.new_dataframe[new_column_name] = \
    ...
    def _ensure_generated_columns(self, dataframe):
        df = dataframe.copy()
        for new_column, info in self.generated_binary_info.items():
            if info['source'] in df.columns and new_column not in df.columns:
                df[new_column] = df[info['source']].apply(lambda x: info['values'].index(x) + 1 if x in info['values'] else 0)
            if new_column in df.columns:
                df[new_column] = df[new_column].astype(int)

        for new_column, info in self.generated_categorical_info.items():
            if info['source'] in df.columns and new_column not in df.columns:
                df[new_column] = df[info['source']].apply(lambda x: info['values'].index(x) + 1 if x in info['values'] else 0)
            if new_column in df.columns:
                df[new_column] = df[new_column].astype(int)
    ...
    def transform(self, dataframe=None):
        df = self.new_dataframe.copy() if dataframe is None else dataframe.copy()
        df = self._ensure_generated_columns(df)
        df = df[self._column_order].copy()
    ...
    def original_columns(self): return self._original_columns


def convert_to_table(org_df, gen_output, threshold, parser=None):
    ...
    parser_conv = DataFrameParser().fit(org_df, threshold) if parser is None else parser
    ...
    sample_size = None
    if 'nums' in gen_output and gen_output['nums'] is not None:
        sample_size = gen_output['nums'].shape[0]
    elif 'cats' in gen_output and len(gen_output['cats']) > 0:
        sample_size = gen_output['cats'][0].shape[0]
    elif 'bins' in gen_output and gen_output['bins'] is not None:
        sample_size = gen_output['bins'].shape[0]
    else:
        sample_size = org_df.shape[0]

    bin_tensor = torch.empty(sample_size, n_bins) if n_bins else torch.empty(sample_size, 0)
    cat_tensor = torch.empty(sample_size, n_cats) if n_cats else torch.empty(sample_size, 0)
    num_tensor = torch.empty(sample_size, n_nums) if n_nums else torch.empty(sample_size, 0)

    if n_nums != 0 and 'nums' in gen_output:
        num_tensor = gen_output['nums'].detach().cpu()

    if len(cards) != 0 and 'cats' in gen_output:
        for idx in range(len(cards)):
            cat_tensor[:,idx] = softmax_with_max(gen_output['cats'][idx]).detach().cpu().to(torch.int64)

    if n_bins != 0 and 'bins' in gen_output:
        for idx in range(n_bins):
            bin_tensor[:,idx] = sigmoid_threshold(gen_output['bins'][:,idx]).detach().cpu().to(torch.int64)

    tensors_to_concat = []
    if n_bins:
        tensors_to_concat.append(bin_tensor)
    if len(cards):
        tensors_to_concat.append(cat_tensor)
    if n_nums:
        tensors_to_concat.append(num_tensor)

    if len(tensors_to_concat) == 0:
        syn_data = torch.empty(sample_size, 0)
    else:
        syn_data = torch.cat(tensors_to_concat, dim=1)
