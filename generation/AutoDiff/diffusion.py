...
def drift_coeff(x, t, beta_1, beta_0):
   t = t.detach().clone() if isinstance(t, torch.Tensor) else torch.tensor(t)
...
def Euler_Maruyama_sampling(model, T, N, P, device, show_progress=True):
    ...
    tqdm_epoch = trange(T, disable=not show_progress, leave=show_progress)
