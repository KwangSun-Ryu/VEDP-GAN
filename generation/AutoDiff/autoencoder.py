...
def train_autoencoder(df, hidden_size, num_layers, lr, weight_decay, n_epochs, batch_size, threshold, is_balanced=False, target_col=None, parser=None, transformed_data=None, show_progress=True):
    if parser is None:
        parser = pce.DataFrameParser().fit(df, threshold)

    if transformed_data is None:
        source_df = df if df is not None else parser.new_df
        data = parser.transform(source_df)
        data = torch.tensor(data.astype('float32'))
    else:
        data = torch.tensor(transformed_data.astype('float32'))
    ...
    tqdm_epoch = trange(n_epochs, disable=not show_progress, leave=show_progress)
    ...
    for epoch in tqdm_epoch:
        ...
        if show_progress:
            tqdm_epoch.set_description('AE Average Loss: {:5f}'.format(l2_loss.item()))
    ...
    latent_features = DS.featurize(data).detach().clone()
    output = DS.decoder(latent_features, num_min_values, num_max_values)

    return (DS.decoder, latent_features, num_min_values.detach().clone(), num_max_values.detach().clone())
