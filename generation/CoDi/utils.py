...
def log_sample_categorical(logits, num_classes):
    ...
    for i in range(len(num_classes)):
        ...
        col_t = np.zeros(logits_column.shape, dtype=np.float32)
        col_t[np.arange(logits_column.shape[0]), sample.detach().cpu()] = 1
        full_sample.append(col_t)
    full_sample = torch.tensor(
        np.concatenate(full_sample, axis=1),
        dtype=torch.float32,
        device=logits.device
    )
    log_sample = torch.log(full_sample.clamp(min=1e-30))
    return log_sample
...
def training_with(x_0_con, x_0_dis, trainer, trainer_dis, ns_con, ns_dis, trans, FLAGS, has_con=True):
    
    device = x_0_dis.device
    batch_size = x_0_dis.shape[0]

    t = torch.randint(FLAGS.T, size=(batch_size, ), device=device)
    ...
    if has_con:
        ...
        eps = trainer.model(x_t_con, t, x_t_dis.to(x_t_con.device, dtype=x_t_con.dtype))
        ps_0_con = trainer.predict_xstart_from_eps(x_t_con, t, eps=eps)
        con_loss = F.mse_loss(eps, noise, reduction='none').mean()
    else:
        x_t_con = torch.zeros(batch_size, 0, device=device)
        ps_0_con = x_t_con
    ...
    if has_con:
        ...
        eps_ns = trainer.model(x_t_con, t, ns_t_dis.to(x_t_con.device, dtype=x_t_con.dtype))
        ns_0_con = trainer.predict_xstart_from_eps(x_t_con, t, eps=eps_ns)
    else:
        ns_t_con = torch.zeros(batch_size, 0, device=device)
        ns_0_con = ns_t_con
    ...
    triplet_con = triplet_loss(x_0_con, ps_0_con, ns_0_con) if has_con else torch.tensor(0.0, device=device)
    st=0
    triplet_dis = []
...
def make_negative_condition(x_0_con, x_0_dis):
    ...
    ns_con = nsc.astype(np.float32)
    ns_dis = nsd.astype(np.float32)

    return torch.tensor(ns_con, dtype=torch.float32, device=device), torch.tensor(ns_dis, dtype=torch.float32, device=device)
