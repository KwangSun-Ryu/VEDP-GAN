...
class tabularUnet(nn.Module):
  def __init__(self, FLAGS):
    ...
    cond_out = max(FLAGS.input_size // 2, 0)
    if FLAGS.input_size > 0 and cond_out < 2:
  ...
  def forward(self, x, time_cond, cond):
    ...
    if self.empty_mode:
      batch = x.shape[0] if x.numel() > 0 else cond.shape[0]
      device = x.device if x.numel() > 0 else cond.device
      dtype = x.dtype if x.numel() > 0 else cond.dtype
      return torch.zeros((batch, self.output_size), device=device, dtype=dtype)
