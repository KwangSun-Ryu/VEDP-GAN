...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False, exp_dir=False, spl=True, writer=None, alpha0=None, beta0=None):
    if continuous:
        ...
        if train:
            ...
            if spl:
                ...
                step_tensor = torch.tensor(
                    state['step'], device=nll.device, dtype=nll.dtype)
                alpha_base = torch.tensor(
                    alpha0, device=nll.device, dtype=nll.dtype)
                beta_base = torch.tensor(
                    beta0, device=nll.device, dtype=nll.dtype)
                q_alpha = torch.clamp(
                    alpha_base + torch.log1p(0.0001718 * step_tensor * (1 - alpha_base)), max=1.)
                q_beta = torch.clamp(
                    beta_base + torch.log1p(0.0001718 * step_tensor * (1 - beta_base)), max=1.)
                q_alpha_val = q_alpha.item()
                q_beta_val = q_beta.item()
                if writer is not None:
                    writer.add_scalars(
                        "quatiles", {"q_alpha": q_alpha_val, "q_beta": q_beta_val}, state['step'])
                ...
                if writer is not None:
                    writer.add_scalars(
                        "thresholds", {"alpha": alpha_val, "beta": beta_val}, state['step'])

                one_mask = torch.isclose(v, torch.ones_like(v))
                zero_mask = torch.isclose(v, torch.zeros_like(v))
                total = int(v.numel())
                selected = int(one_mask.sum().item())
                dropped = int(zero_mask.sum().item())
                weighted = int(total - selected - dropped)
                state['last_metrics'] = {
...
def compute_v(ll, alpha, beta):
    ...
    if ll[torch.eq(v, -1)].shape[0] != 0 and ll[torch.eq(v, -1)].shape[0] != 1:
        v[torch.eq(v, -1)] = min_max_scaling(ll[torch.eq(v, -1)],
                                             scale=(1, 0)).to(v.device)
    else:
