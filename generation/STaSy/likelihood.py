...
def get_div_fn(fn):
    ...
    def div_fn(x, t, eps):
        ...
        for epsilon in eps:
            ...
            grad_fn_eps_list.append(
                torch.sum(grad_fn_eps * epsilon, dim=tuple(range(1, len(x.shape)))))
        ...
        with torch.no_grad():
            ...
            if hutchinson_type == 'Gaussian':
                ...
                epsilon = [torch.randint_like(
                    data, low=0, high=2).float() * 2 - 1. for k in range(eps_iters)]
            else:
            ...
            z = mutils.from_flattened_numpy(
                zp[:-shape[0]], shape).to(data.device).type(torch.float64)
            delta_logp = mutils.from_flattened_numpy(
                zp[-shape[0]:], (shape[0],)).to(data.device).type(torch.float64)
            prior_logp = sde.prior_logp(z).view(
                shape[0], -1).sum(1, keepdim=False)
