...
def _weighted_f1(y_test, pred):
    ...
    weighted_f1 = np.sum(list(map(
        lambda i, prop: report[i]['f1-score'] * (1-prop)/(len(classes)-1), classes, proportion)))
    return weighted_f1


@ignore_warnings(category=ConvergenceWarning)
def _evaluate_multi_classification(train, test, fake, metadata):
    x_trains, y_trains, x_valid, y_valid, x_test, y_test, classifiers = _prepare_ml_problem(
        fake, train, test, metadata)
    best_f1_scores = []
    ...
    for model_spec in classifiers:
        ...
        for param in param_set:
            ...
            # 3. auroc
            size = [a["size"]
                    for a in metadata["columns"] if a["name"] == "label"][0]
        ...
        results['avg'] = results.loc[:, ['macro_f1',
                                         'weighted_f1', 'roc_auc']].mean(axis=1)
        ...
        def _calc(best_model):
            ...
            for x_train, y_train in zip(x_trains, y_trains):
                ...
                for i in range(size):
                    if i in rest_label:
                        tmp.append(
                            np.array([0] * y_test.shape[0])[:, np.newaxis])
                    else:
                        try:
                            tmp.append(pred_prob[:, [j]])
                        except:
        ...
        best_weighted_scores.append(
            _df(_calc(model_class(**best_weighted_param))))
...
def _evaluate_binary_classification(train, test, fake, metadata):
    ...
    for model_spec in classifiers:
        ...
        for param in param_set:
            ...
            binary_f1 = f1_score(
                y_valid, pred, average='binary', zero_division=0)
            weighted_f1 = _weighted_f1(y_valid, pred)
            acc = accuracy_score(y_valid, pred)
        ...
        # test the best model
        results = pd.DataFrame(results)
        results['avg'] = results.loc[:, ['binary_f1',
                                         'weighted_f1', 'roc_auc']].mean(axis=1)
        ...
        def _calc(best_model):
            ...
            for x_train, y_train in zip(x_trains, y_trains):
                ...
                try:
                    roc_auc = roc_auc_score(
                        np.eye(size)[y_test], np.hstack(tmp))
                except ValueError:
...
def _evaluate_regression(train, test, fake, metadata):
    x_trains, y_trains, x_valid, y_valid, x_test, y_test, regressors = _prepare_ml_problem(
        fake, train, test, metadata)
    ...
    for model_spec in regressors:
        ...
        for param in param_set:
            ...
            root_mean_squared = mean_squared_error(
                y_valid, pred, squared=False)
...
def compute_scores(train, test, synthesized_data, metadata):
    a, b, c = _EVALUATORS[metadata['problem_type']](
        train=train, test=test, fake=synthesized_data, metadata=metadata)
    numeric_a = a.select_dtypes(
        include=[np.number]) if isinstance(a, pd.DataFrame) else a
    return numeric_a.mean(axis=0), numeric_a.std(axis=0)
